...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    const setupChat = async () => {
      ...
      if (chatError) {
        console.error('خطأ في جلب المحادثة:', chatError.message)
        return
      }
      ...
      if (messagesError) {
        console.error('خطأ في جلب الرسائل:', messagesError.message)
        return
      }

      setMessages(oldMessages || [])

      // تعليم الرسائل كمقروءة عند الدخول
...
<NotificationSetup />
      <div className="p-4 bg-zinc-900 border-b border-zinc-800 flex items-center justify-between shadow-xl">
        ...
        <div className="text-center flex flex-col items-center">
          ...
          <button
            ...
            className="text-[10px] bg-red-500/10 text-red-500 px-3 py-1 rounded-full font-bold mt-1"
          >
            إبلاغ عن مخالفة
          </button>
        </div>
        ...
        <button
          onClick={shareMyPhone}
          className="bg-zinc-800 p-2 rounded-xl text-orange-500 hover:bg-orange-500 hover:text-black transition-all"
        >
          <Phone size={20} />
        </button>
      </div>
      ...
      <form onSubmit={sendMessage} className="p-4 bg-zinc-900 border-t border-zinc-800 flex gap-2">
        ...
        <button
          type="submit"
          className="bg-orange-500 text-black p-4 rounded-2xl hover:scale-105 transition-transform active:scale-95 shadow-[0_0_15px_rgba(249,115,22,0.3)]"
        >
          <Send size={20} />
        </button>
      </form>
